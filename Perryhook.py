...
async def main(webhook, username, avatar, delay, amount, message, deleter):
    ...
    while True if amount == "inf" else counter < int(amount):
        ...
        if data.status_code == 204:
            ...
            # Checks during sending stuff if the webhook was deleted and if it was it alerts u so u know u aren't wasting ur time.
        if "\"message\": \"Unknown Webhook\"" in requests.get(webhook).text:
...
╚═╝░░░░░╚══════╝╚═╝░░╚═╝╚═╝░░╚═╝░░░╚═╝░░░╚═╝░░╚═╝░╚════╝░░╚════╝░╚═╝░░╚═╝
  I love Perryhook.
    ...
    # Checks the length of the username input and if its over 80 it alerts u because in normal Discord without nitro that wouldn't work and usernames thru webhooks can't have nitro, so it limits to 80 characters and simply closes/exits the program.
    if len(username) > 80:
        print(
    ...
    # Checks the length of the message input and if its over 2000 it alerts u because in normal Discord without nitro that wouldn't work and sending messages thru webhooks can't have nitro, so it limits to 2000 characters and simply closes/exits the program.
    if len(message) > 2000:
        print(
